...
if (!function_exists('accalia_woocommerce_theme_setup1')) {
	...
	function accalia_woocommerce_theme_setup1() {
		...
		add_theme_support( 'woocommerce'/*, array( 'product_grid' => array( 'max_columns' => 6 ) )*/ );
...
if (!function_exists('accalia_woocommerce_theme_setup3')) {
	...
	function accalia_woocommerce_theme_setup3() {
		if (accalia_exists_woocommerce()) {
			...
			// Section 'WooCommerce'
			accalia_storage_set_array_before('options', 'fonts', array_merge(
				array(
					'shop' => array(
...
if (!function_exists('accalia_woocommerce_theme_setup9')) {
	...
	function accalia_woocommerce_theme_setup9() {
		...
		if (accalia_exists_woocommerce()) {
			...
			add_filter( 'accalia_filter_get_blog_title', 'accalia_woocommerce_get_blog_title' );

			// Add 'Out of stock' label
			add_action( 'accalia_action_woocommerce_item_featured_link_start', 'accalia_woocommerce_add_out_of_stock_label' );
	...
	function accalia_woocommerce_item_wrapper_start($cat='') {
		...
		<div class="post_item post_layout_<?php echo esc_attr(accalia_storage_get('shop_mode')); ?>">
			<div class="post_featured hover_<?php echo esc_attr($hover); ?>">
				...
				<?php
				do_action( 'accalia_action_woocommerce_item_featured_link_start' );
	}
}
...
if ( ! function_exists( 'accalia_woocommerce_add_out_of_stock_label' ) ) {
	function accalia_woocommerce_add_out_of_stock_label() {
		global $product;
		$cat = accalia_storage_get( 'in_product_category' );
		if ( empty($cat) || ! is_object($cat) ) {
			if ( is_object( $product ) && ! $product->is_in_stock() ) {
				?>
				<span class="outofstock_label"><?php esc_html_e( 'Out of stock', 'accalia' ); ?></span>
				<?php
			}
		}
	}
...
if ( ! function_exists( 'accalia_woocommerce_get_blog_title' ) ) {
	//Handler of the add_filter( 'accalia_filter_get_blog_title', 'accalia_woocommerce_get_blog_title');
	function accalia_woocommerce_get_blog_title( $title = '' ) {
		if ( ! accalia_exists_trx_addons() && accalia_exists_woocommerce() && accalia_is_woocommerce_page() && is_shop() ) {
			$id    = accalia_woocommerce_get_shop_page_id();
			$title = $id ? get_the_title( $id ) : esc_html__( 'Shop', 'accalia' );
		}
		return $title;
	}
}
