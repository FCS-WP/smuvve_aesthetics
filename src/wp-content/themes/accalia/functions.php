...
if ( !function_exists('accalia_add_body_classes') ) {
	function accalia_add_body_classes( $classes ) {
		...
		$classes[] = 'trx_addons_' . esc_attr(accalia_exists_trx_addons() ? 'present' : 'absent');
...
if ( !function_exists( 'accalia_importer_set_options' ) ) {
	function accalia_importer_set_options($options=array()) {
		if (is_array($options)) {
			...
			$options['files']['default']['domain_dev'] = esc_url(accalia_get_protocol() . '://amelia.dv.ancorathemes.com');		// Developers domain
			$options['files']['default']['domain_demo']= esc_url(accalia_get_protocol() . '://accalia.ancorathemes.com');		// Demo-site domain
		}
		return $options;
	}
}
...
if ( ! function_exists( 'accalia_comment_form_agree' ) ) {
	add_filter('comment_form_fields', 'accalia_comment_form_agree', 11);
	function accalia_comment_form_agree( $comment_fields ) {
		 $privacy_text = accalia_get_privacy_text();
		 if ( ! empty( $privacy_text ) 
			&& ( ! function_exists( 'accalia_exists_gdpr_framework' ) || ! accalia_exists_gdpr_framework() )
			&& ( ! function_exists( 'accalia_exists_wp_gdpr_compliance' ) || ! accalia_exists_wp_gdpr_compliance() )
		 ) {
			  $comment_fields['i_agree_privacy_policy'] = accalia_single_comments_field(
					array(
						 'form_style'        => 'default',
						 'field_type'        => 'checkbox',
						 'field_req'         => '',
...
if (is_admin()) {
	require_once ACCALIA_THEME_DIR . 'includes/fixes/fix_urls.php';
}
