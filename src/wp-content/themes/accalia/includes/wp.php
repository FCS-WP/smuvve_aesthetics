	...
	function accalia_get_post_terms($delimiter=', ', $id=false, $taxonomy='category') {
		...
		if ( !empty( $terms ) ) {
			foreach( $terms as $term )
				$output .= ($output ? $delimiter : '') . '<a href="' . esc_url( get_term_link( $term->term_id, $taxonomy ) ) . '" title="' . sprintf( esc_attr__( 'View all posts in %s', 'accalia' ), esc_attr($term->name) ) . '">' . esc_html( $term->name ) . '</a>';
		}
		return $output;
	}
}
...
if ( ! function_exists( 'accalia_get_post_id' ) ) {
    function accalia_get_post_id( $args = array() ) {
        $args  = array_merge(
            array(
                'posts_per_page' => 1,
            ), $args
