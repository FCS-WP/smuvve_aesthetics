...
(function() {
		...
		if (container.find('.sc_slider_controller:not(.inited)').length > 0) {
			container.find('.sc_slider_controller:not(.inited)')
				.each(function () {
					...
					jQuery('#'+slider_id+' .slider_container .slider_swiper').attr('data-controller', controller_id);
	...
	function trx_addons_resize_sliders(e, container) {
		...
		container.find('.slider_container.inited, .slider_swiper.inited').each(function() {
			...
			if (last_width==0 || last_width!=slider_width) {
				...
				if ( !slider.hasClass('slider_noresize') || slider.height()==0 ) {
					var slide = slider.find('.slider-slide, .swiper-slide').eq(0);
		...
		if (controller && controller.length > 0
			...
			if (jQuery(window).width() >= 768) {
				...
				if (isNaN(paddings)) paddings = 0;
								controller.find('.slider_container, .slider_swiper').height(slider.height() - 2*paddings);
			} else {
				var controller_spv = controller.data('slides-per-view');
				if (isNaN(controller_spv)) controller_spv = 1;
								controller.find('.slider_container, .slider_swiper').height(controller_spv*100);
			}
		}
	}
