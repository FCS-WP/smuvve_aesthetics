...
class Essential_Grid
{
	...
	{
		$use_cache =  self::$base::getUseCache() == 'true';
	...
	public static function register_shortcode($args, $mid_content = null)
	{
		...
		$content = false;
		$grid_id = Essential_Grid_Db::get_id_by_alias($eg_alias);
	...
	public static function register_shortcode_ajax_target($args, $mid_content = null)
	{
		...
		$content = '';
		$grid = new Essential_Grid;
		$grid_id = Essential_Grid_Db::get_id_by_alias($alias);
	...
	protected static function _process_custom_grid_filter($grid, $filter_selected) {
		...
		foreach ($filter_selected as $fk => $filter) {
			$return['filter-selected'][$fk] = self::$base::sanitize_utf8_to_unicode($filter);
			if (strpos($filter, 'meta-') !== 0) continue;

			// filter is meta
			// unset original slug as we will replace it with meta slug(s)
			unset($return['filter-selected'][$fk]);
			
			$handle = str_replace('meta-', '', $filter);
	...
	protected static function _process_post_filter($grid, $post) {
		...
		// check grid params for filter-selected(-X)
		$filters_in_params = array_filter(
			$grid->grid_params,
			function($k) {
		...
		foreach ($filters_in_params as $f_values) {
			...
			foreach ($f_values as $handle) {
				...
				$fil = str_replace('meta-', '', $handle);
				$filter_meta = self::$meta->get_meta_value_by_handle($post['ID'], 'eg-' . $fil, false);
				if ($filter_meta == '') {
					// check if we are linking
					$filter_meta = self::$meta_link->get_link_meta_value_by_handle($post['ID'], 'egl-' . $fil);
				}
	...
	public static function get_grid_filter($f_id, $grid) {
		...
		$filter_data['filter-all-text'] = self::$base->getVar($grid->grid_params, 'filter-all-text' . $f_id, esc_attr__('Filter - All', ESG_TEXTDOMAIN));
		...
		$filter_data['filter-dropdown-text'] = self::$base->getVar($grid->grid_params, 'filter-dropdown-text' . $f_id, esc_attr__('Filter Categories', ESG_TEXTDOMAIN));
	...
	public static function prepare_filters($nav, $grid, $filters_in_params_layout = []) {
		...
		if (empty($filters_in_params_layout)) {
			// check grid params for filter-selected(-X)
	...
	public static function register_shortcode_filter($args, $mid_content = null)
	{
		...
		$content = '<div class="' . $navigation_c->get_special_class() . '">' . ob_get_contents() . '</div>';
		ob_clean();
		ob_end_clean();
	...
	{
		$id = Essential_Grid_Db::get_id_by_alias($alias);
		if (!empty($id)) {
	...
	public function init_by_id($grid_id)
	{
		...
		$grid = Essential_Grid_Db::get_essential_grid_by_id($grid_id);
	...
	protected function _init_nav($order_by, $order_by_start, $order_by_dir)
	{
		...
		$nav->set_search_text(self::$base->getVar($this->grid_params, 'search-text', esc_attr__('Search...', ESG_TEXTDOMAIN)));
		$nav->set_layout(self::$base->getVar($this->grid_params, 'navigation-layout', [])); //set the layout

		$nav->set_orders($order_by);
		$nav->set_orders_text(self::$base->getVar($this->grid_params, 'sort-by-text', esc_attr__('Sort By ', ESG_TEXTDOMAIN)));
	...
	protected function _init_item_skin()
	{
		...
		$item_skin->set_fancybox_three_options(self::$base->getVar($this->grid_params, 'lightbox-title', 'off'));
	...
	public function output_by_posts($grid_preview = false, $only_elements = false, $set_load_more = false, $from_ajax = false)
	{
		...
		$lightbox_include_media = self::$base->getVar($this->grid_params, 'lightbox-exclude-media', 'off');
	...
	public function prepare_sorting_array_by_custom($post, $order_by)
	{
		...
		foreach ($order_by as $order) {
			switch ($order) {
				case 'date':
					$sorts['date'] = strtotime(self::$base->getVar($post, 'date', date('Y-m-d H:i:s')));
					break;
				case 'title':
	...
	public function output_grid_javascript($load_lightbox = false, $is_demo = false)
	{
		...
		if ($lightbox_effect_open_close != 'false')
			$lightbox_effect_open_close = '"' . esc_js($lightbox_effect_open_close) . '"';
		...
		if ($lightbox_effect_next_prev != 'false')
			$lightbox_effect_next_prev = '"' . esc_js($lightbox_effect_next_prev) . '"';
		...
		if ($ajax_button_type == 'type2') {
			$ajax_button_text = self::$base->getVar($this->grid_params, 'ajax-button-text', esc_attr__('Close', ESG_TEXTDOMAIN));
		}
		...
		if ($hide_markup_before_load == 'off') {
			...
			echo '  }' . "\n";
			echo '  var coh=0,' . "\n";
			echo '    container = jQuery("#esg-grid-' . $grid_div_serial . '");' . "\n";
			if ($layout_sizing == 'fullscreen') {
		...
		echo 'var essapi_' . $grid_api_serial . ';' . "\n";
		echo 'window.ESG ??={};' . "\n";
		...
		echo 'window.ESG.inits.v' . $grid_api_serial . ' ??={ state:false};' . "\n";
		echo 'window.ESG.inits.v' . $grid_api_serial . '.call = () => {' . "\n";
		...
		echo '        wheel : ' . esc_js($lightbox_mousewheel) . ',' . "\n";
		...
		echo '  jQuery("#esg-grid-' . $grid_div_serial . '").data("lightboxsettings", lightboxOptions);' . "\n\n";

		// lightbox:end
		///////////////
		
		echo '  essapi_' . $grid_api_serial . ' = jQuery("#esg-grid-' . $grid_div_serial . '").tpessential({' . "\n";
		...
		if ($rows_unlimited == 'on') {
			...
			if ($load_more !== 'none') {
				$load_more_text = self::$base->getVar($this->grid_params, 'load-more-text', esc_attr__('Load More', ESG_TEXTDOMAIN));
		...
		echo '        apiName: "essapi_' . $grid_api_serial . '",' . "\n";
		...
		echo ' if ((document.readyState === "interactive" || document.readyState === "complete") && !window.ESG.inits.v' . $grid_api_serial . '.state )';
		echo '  {';
		echo ' if ((jQuery?.fn?.tpessential)) {';		
		echo '   window.ESG.inits.v' . $grid_api_serial . '.state = true;';
		echo '   window.ESG.inits.v' . $grid_api_serial . '.call();';
		echo '}';
		echo '  }';
		echo '});';
  ...
  #' . esc_attr($grid_id_wrap) . '.eg-startheight{ height: ' . esc_attr($height) . 'px; }
	...
	public static function _uninstall_plugin()
	{
		...
		$wpdb->query("DROP TABLE " . Essential_Grid_Db::get_table('grids', true));
		$wpdb->query("DROP TABLE " . Essential_Grid_Db::get_table('skins', true));
		$wpdb->query("DROP TABLE " . Essential_Grid_Db::get_table('elements', true));
		$wpdb->query("DROP TABLE " . Essential_Grid_Db::get_table('nav_skins', true));
	...
	public static function on_lightbox_post_content($settings, $id)
	{
		...
		if (!empty($settings)) {
			...
			if (!in_array($settings['lbTag'], ['h1', 'h2', 'h3', 'h4', 'h5', 'h6'])) $settings['lbTag'] = 'h2';
			...
			if (!empty($featured) && $lbImg === 'on') {
				...
				$featured = '<img class="esg-post-featured-img" src="' . esc_url($featured) . '" style="width: 100%; height: auto; padding: ' . $margin . '" />';
		...
		if ($isVerified) {
			...
			switch ($action) {
				...
				case 'get_grid_search_ids':
					$search_string = self::$base->getVar($data, 'search');
					$grid_id = self::$base->getVar($data, 'id', 0, 'i');
	...
	public function add_admin_menu_nodes()
	{
		...
		$arrGrids = Essential_Grid_Db::get_essential_grids();
	...
	{
		_deprecated_function( __METHOD__, '3.1.3', 'Essential_Grid_Db::get_essential_grids()' );
		return Essential_Grid_Db::get_essential_grids($order, $raw);
	}
	...
	{
		_deprecated_function( __METHOD__, '3.1.3', 'Essential_Grid_Db::get_essential_grid_by_id()' );
		return Essential_Grid_Db::get_essential_grid_by_id($id, $raw);
	}
	...
	{
		_deprecated_function( __METHOD__, '3.1.3', 'Essential_Grid_Db::get_essential_grid_by_handle()' );
		return Essential_Grid_Db::get_essential_grid_by_handle($handle, $raw);
	}
	...
	{
		_deprecated_function( __METHOD__, '3.1.3', 'Essential_Grid_Db::get_grids_short()' );
		return Essential_Grid_Db::get_grids_short($exceptID);
	}
	...
	{
		_deprecated_function( __METHOD__, '3.1.3', 'Essential_Grid_Db::get_grids_short_widgets()' );
		return Essential_Grid_Db::get_grids_short_widgets($exceptID);
	}
	...
	{
		_deprecated_function( __METHOD__, '3.1.3', 'Essential_Grid_Db::get_grids_short_vc()' );
		return Essential_Grid_Db::get_grids_short_vc($exceptID);
	}
