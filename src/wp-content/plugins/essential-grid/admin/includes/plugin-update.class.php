...
class Essential_Grid_Plugin_Update
{
	...
	public function do_update_process()
	{
		...
		if (version_compare($this->version, '3.1.3', '<')) {
			$this->update_version('3.1.3');
			$this->set_version('3.1.3');
		}
...
.simple-dark input.eg-search-input[type="text"] { box-sizing: border-box;
	...
	*/
	...
	public function insert_skin($skin)
	{
		...
		$wpdb->insert($table_name, ['name' => $skin['name'], 'handle' => $skin['handle'], 'params' => $skin['params'], 'layers' => $skin['layers']]);
	}
...
.#handle# .esg-pagination-button:last-child { 
	...
	protected function _update_grids_3014()
	{
		...
		$grids = Essential_Grid_Db::get_essential_grids(false, false);
		foreach ($grids as $grid) {
			$grid->params = self::update_grid_desktop_xl($grid->params);
			$wpdb->update($table_name, ['params' => json_encode($grid->params)], ['id' => $grid->id]);
		}
	}
...
.flat-light .esg-navigationbutton.esg-loadmore {
	...
	protected function _update_3016_watermarks()
	{
		...
		$arrGrids = Essential_Grid_Db::get_essential_grids(false, false);
		...
		Essential_Grid_Db::clear_essential_grids_cache();
